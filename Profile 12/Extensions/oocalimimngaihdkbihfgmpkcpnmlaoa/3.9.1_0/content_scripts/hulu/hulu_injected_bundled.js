...
var seekInteraction = function (event) {
    ...
    switch (event_type) {
        case ("SEEK"): {
            document.querySelector('#content-video-player').__HuluDashPlayer__.currentTime = event.data.time / 1000;
            break;
        }
        case ("UpdateState"): {
            const video = document.querySelector('#content-video-player');
            const paused = video.__HuluDashPlayer__._paused;
            const currentTime = video.__HuluDashPlayer__._position.offset * 1000;
