...
(v=v[v.length-1]),r.message&&r.stack&&-1===r.stack.indexOf(r.message)&&(r=r.message+"\n"+r.stack),Eb("modulerr",v,r.stack||r.message||r);}}});return D}function k(C,D){function y(p,r){if(C.hasOwnProperty(p)){if(C[p]===h)throw Eb("cdep",p+" <- "+l.join(" <- "));return C[p]}try{return l.unshift(p),C[p]=h,C[p]=D(p,r),C[p]}catch(m){throw C[p]===h&&delete C[p],m;}finally{l.shift()}}function v(p,r,m){var x=[];p=gc.$$annotate(p,b,m);for(var G=0,B=p.length;G<B;G++){var A=p[G];if("string"!==typeof A)throw Eb("itkn",
A);x.push(r&&r.hasOwnProperty(A)?r[A]:y(A,m))}return x}return{invoke:function(p,r,m,x){"string"===typeof m&&(x=m,m=null);m=v(p,m,x);oa(p)&&(p=p[p.length-1]);x=p;if(qb||"function"!==typeof x)x=!1;else{var G=x.$$ngIsClass;vb(G)||(G=x.$$ngIsClass=/^(?:class\b|constructor\()/.test(Function.prototype.toString.call(x)));x=G}return x?(m.unshift(null),new (Function.prototype.bind.apply(p,m))):p.apply(r,m)},instantiate:function(p,r,m){var x=oa(p)?p[p.length-1]:p;p=v(p,r,m);p.unshift(null);return new (Function.prototype.bind.apply(x,
