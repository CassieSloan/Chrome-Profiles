...
$jscomp.polyfillIsolated=function(a,b,d){var c=a.split("."),e=1===c.length;a=c[0];a=!e&&a in $jscomp.polyfills?$jscomp.polyfills:$jscomp.global;for(var f=0;f<c.length-1;f++){var g=c[f];if(!(g in a))return;a=a[g]}c=c[c.length-1];d=$jscomp.IS_SYMBOL_NATIVE&&"es6"===d?a[c]:null;b=b(d);null!=b&&(e?$jscomp.defineProperty($jscomp.polyfills,c,{configurable:!0,writable:!0,value:b}):b!==d&&(void 0===$jscomp.propertyToPolyfillSymbol[c]&&(e=1E9*Math.random()>>>0,$jscomp.propertyToPolyfillSymbol[c]=$jscomp.IS_SYMBOL_NATIVE?
...
$jscomp.polyfill("Symbol",function(a){if(a)return a;var b=function(f,g){this.$jscomp$symbol$id_=f;$jscomp.defineProperty(this,"description",{configurable:!0,writable:!0,value:g})};b.prototype.toString=function(){return this.$jscomp$symbol$id_};a=1E9*Math.random()>>>0;var d="jscomp_symbol_"+a+"_",c=0,e=function(f){if(this instanceof e)throw new TypeError("Symbol is not a constructor");return new b(d+(f||"")+"_"+c++,f)};return e},"es6","es3");
...
function(S,K,M,I,J){if("foreground"===K)return"shadow"==M?D.foregroundShadow:D.foregroundPalette[M]||D.foregroundPalette["1"];if(0===M.indexOf("hue")||"default"===M)M=D.colors[K].hues[M];return n((r[D.colors[K].name][M]||"")[J?"contrast":"value"],I)});var F=RegExp("('|\")?{{\\s*([a-zA-Z]+)-(color|contrast)-?(\\d\\.?\\d*)?\\s*}}(\"|')?","g"),O=[];a.forEach(["default","hue-1","hue-2","hue-3"],function(S){var K=N.replace(F,function(I,J,Q,T,Y){Q=D.colors[Q];I=r[Q.name];Q=Q.hues[S];return n(I[Q]["color"===
T?"value":"contrast"],Y)});"default"!==S&&(K=K.replace(C,".md-"+D.name+"-theme.md-"+S));if("default"==D.name){var M=/((?:\s|>|\.|\w|-|:|\(|\)|\[|\]|"|'|=)*)\.md-default-theme((?:\s|>|\.|\w|-|:|\(|\)|\[|\]|"|'|=)*)/g;K=K.replace(M,function(I,J,Q){return I+", "+J+Q})}O.push(K)});return O}function h(D,G){function N(F){var O=F.contrastDefaultColor,S=F.contrastLightColors||[],K=F.contrastStrongLightColors||[],M=F.contrastDarkColors||[];"string"===typeof S&&(S=S.split(" "));"string"===typeof K&&(K=K.split(" "));
"string"===typeof M&&(M=M.split(" "));delete F.contrastDefaultColor;delete F.contrastLightColors;delete F.contrastStrongLightColors;delete F.contrastDarkColors;a.forEach(F,function(I,J){if(!a.isObject(I)){var Q=v(I);if(!Q)throw Error("Color %1, in palette %2's hue %3, is invalid. Hex or rgb(a) color expected.".replace("%1",I).replace("%2",F.name).replace("%3",J));F[J]={hex:F[J],value:Q,contrast:"light"===O?-1<M.indexOf(J)?k:-1<K.indexOf(J)?t:p:-1<S.indexOf(J)?-1<K.indexOf(J)?t:p:k}}})}var C=document.head;
C=C?C.firstElementChild:null;D=!u.disableTheming&&D.has("$MD_THEME_CSS")?D.get("$MD_THEME_CSS"):"";D+=u.registeredStyles.join("");C&&0!==D.length&&(a.forEach(r,N),D=D.split(/\}(?!(\}|'|"|;))/).filter(function(F){return F&&F.trim().length}).map(function(F){return F.trim()+"}"}),H.forEach(function(F){z[F]=""}),D.forEach(function(F){for(var O=0,S;S=H[O];O++)if(-1<F.indexOf(".md-"+S))return z[S]+=F;for(O=0;S=H[O];O++)if(-1<F.indexOf(S))return z[S]+=F;return z.primary+=F}),u.generateOnDemand||a.forEach(G.THEMES,
function(F){q[F.name]||"default"!==G.defaultTheme()&&"default"===F.name||l(F,F.name,u.nonce)}))}function l(D,G,N){var C=document.head,F=C?C.firstElementChild:null;q[G]||(H.forEach(function(O){for(O=g(D,O,z[O]);O.length;){var S=O.shift();if(S){var K=document.createElement("style");K.setAttribute("md-theme-style","");N&&K.setAttribute("nonce",N);K.appendChild(document.createTextNode(S));C.insertBefore(K,F)}}}),q[D.name]=!0)}function m(D,G){if(!r[(D.colors[G]||{}).name])throw Error("You supplied an invalid color palette for theme %1's %2 palette. Available palettes: %3".replace("%1",
